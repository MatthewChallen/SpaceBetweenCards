...
public class GameWindow extends JPanel {
	...
	private void paintPlayerHand(Graphics screen, int xPosition, int yPosition, int xSize, int ySize) {
		...
		if(heightConstraint >= widthConstraint) {
			//If the cards are more constrained by height, base the cards on the available height
			cardHeight = ySize;
			...
			if(fileName != null) {
				...
				if(!foundSprite) {
					if(loadNewSprite(fileName)) {
						...
						currentXPosition = xPosition + cardNo * cardWidth;
						spriteList.get(spriteList.size()-1).paint(screen, currentXPosition, yPosition, cardHeight, cardWidth);
					}
				}
			}
		}
				...
				if(fileName != null) {
					...
					for(int spritePosition = 0; spritePosition < spriteList.size(); spritePosition++) {
						if(fileName.equals(spriteList.get(spritePosition).getName())) {
							...
							spriteList.get(spritePosition).paint(screen, currentXPosition, currentYPosition, spriteHeight, spriteWidth);
						}
					}
